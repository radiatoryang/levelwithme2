    ...
    void OnGUI() {
        ...
        if ( currentURL != null ) {
            ...
            if ( guiPos.x > Screen.width - width )
                guiPos.x -= width;
			if ( guiPos.y > Screen.height - height)
				guiPos.y -= height;
            GUI.color = new Color( 1f, 1f, 1f, 0.1f );
            GUI.DrawTexture( new Rect( guiPos.x - padding, guiPos.y - padding * 0.5f, width + padding * 2f, height + (Application.isWebPlayer ? 20f : 0f) ), texture );
			...
			if ( Application.isWebPlayer ) {
				GUI.Label ( new Rect( guiPos.x, guiPos.y + height * 0.8f, width, height), "<b><size=12>(ENABLE POPUPS IN YOUR BROWSER.)</size></b>");
			}
		}
    }
